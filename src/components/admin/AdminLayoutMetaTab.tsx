  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={() => handleSave()}
            disabled={isSaving}
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#012A22] to-[#3B796A] hover:from-[#001F18] hover:to-[#012A22] text-white px-5 py-2.5 text-xs sm:text-sm font-bold shadow-lg shadow-[#012A22]/30 transition-all cursor-pointer active:scale-98 disabled:opacity-60"
          >
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {/* ========================================================= */}
        <div className="rounded-2xl border border-slate-800 bg-[#0e111a] p-5 sm:p-6 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <Search className="h-4 w-4 text-sky-400" />
              <h3 className="text-sm font-bold text-white">Google Search Result (SERP) Live Preview</h3>
            </div>
            <div className="flex items-center gap-1.5 bg-slate-900/90 rounded-lg p-1 border border-slate-800 text-[11px]">
              <button
                type="button"
                onClick={() => setPreviewDevice("desktop")}
                className={`px-2.5 py-1 rounded-md font-semibold transition-all cursor-pointer ${
                  previewDevice === "desktop" ? "bg-slate-800 text-white" : "text-slate-400 hover:text-slate-200"
                }`}
              >
                Desktop
              </button>
              <button
                ...
                className={`px-2.5 py-1 rounded-md font-semibold transition-all cursor-pointer ${
                  previewDevice === "mobile" ? "bg-slate-800 text-white" : "text-slate-400 hover:text-slate-200"
                }`}
              >
                Mobile
              </button>
          ...
          <div
            className={`rounded-xl border border-slate-700/80 bg-[#161a23] p-4 sm:p-5 transition-all ${
              previewDevice === "mobile" ? "max-w-md mx-auto" : "w-full"
            }`}
          >
            <div className="flex items-center gap-2 mb-1.5">
              <div className="h-6 w-6 rounded-full bg-[#012A22] flex items-center justify-center text-white text-[10px] font-black shrink-0">
                T
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-[12px] font-medium text-slate-300 leading-tight">
            ...
            <h4 className="text-[17px] sm:text-[19px] font-medium text-[#8ab4f8] hover:underline cursor-pointer leading-snug line-clamp-1">
              {settings.siteTitle || "TREQO"} — {settings.titleTemplate?.replace("%s | ", "") || "Digital Marketing School"}
            </h4>

            {/* Google Snippet Description */}
            <p className="mt-1 text-[13px] text-[#bdc1c6] leading-relaxed line-clamp-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            <div className="flex items-center justify-between text-xs px-3 py-2 rounded-xl bg-slate-900/60 border border-slate-800">
              ...
              <span
                className={`font-mono font-bold ${
                  titleLength > 60 ? "text-amber-400" : titleLength > 30 ? "text-emerald-400" : "text-slate-400"
                }`}
              >
            ...
            <div className="flex items-center justify-between text-xs px-3 py-2 rounded-xl bg-slate-900/60 border border-slate-800">
              ...
              <span
                className={`font-mono font-bold ${
                  descLength > 160 ? "text-amber-400" : descLength > 120 ? "text-emerald-400" : "text-slate-400"
                }`}
              >
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#0e111a] p-5 sm:p-6 space-y-4">
          <div className="flex items-center gap-2">
            <Layers className="h-4 w-4 text-[#ABCAC2]" />
            <h3 className="text-sm font-bold text-white">Title &amp; Meta Description</h3>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-bold text-slate-300">
                  Site Meta Title (Default) <span className="text-red-400">*</span>
                </label>
                <span className="text-[11px] text-slate-500 font-mono">{titleLength}/60</span>
              </div>
              <input
                type="text"
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white focus:border-[#012A22] focus:outline-none placeholder:text-slate-500 font-medium"
              />
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-bold text-slate-300">Subpage Title Template</label>
              </div>
              <input
                type="text"
              ...
              />
              <p className="mt-1 text-[11px] text-slate-500">
                <code className="bg-slate-900 px-1 py-0.5 rounded text-[#ABCAC2]">%s</code> represents the subpage title (e.g. &ldquo;Digital Marketing Course | TREQO&rdquo;).
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[11px] text-slate-500 font-mono">{descLength}/160</span>
              </div>
              <textarea
                rows={3}
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white focus:border-[#012A22] focus:outline-none placeholder:text-slate-500 leading-relaxed font-medium"
              />
              <p className="mt-1 text-[11px] text-slate-500">
            ...
            <div>
              <label className="text-xs font-bold text-slate-300">Author / Publisher Name</label>
              <input
                type="text"
                value={settings.authorName || ""}
                onChange={(e) => setSettings({ ...settings, authorName: e.target.value })}
                placeholder="e.g. Treqo School of Modern Learning"
                className="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white focus:border-[#012A22] focus:outline-none placeholder:text-slate-500 font-medium"
              />
            </div>
          </div>
        </div>
        ...
        {/* ========================================================= */}
        <div className="rounded-2xl border border-slate-800 bg-[#0e111a] p-5 sm:p-6 space-y-5">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <Tag className="h-4 w-4 text-emerald-400" />
              <h3 className="text-sm font-bold text-white">Meta Keywords Manager</h3>
              <span className="ml-1.5 rounded-md bg-emerald-950/80 border border-emerald-700/40 text-emerald-300 px-2 py-0.5 text-[10px] font-bold">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white px-4 py-2.5 text-xs sm:text-sm font-bold transition-all cursor-pointer shrink-0 active:scale-98"
            >
          ...
          {showRawKeywords ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-500">Separate keywords with commas</span>
              </div>
              <textarea
                rows={4}
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-xs sm:text-sm text-white focus:border-[#012A22] focus:outline-none font-mono leading-relaxed"
              />
            </div>
          ) : (
            /* Visual Interactive Tag Pills */
            <div className="flex flex-wrap gap-2 p-3.5 rounded-xl border border-slate-800/80 bg-slate-900/50 min-h-[70px]">
              {settings.metaKeywords.length > 0 ? (
                settings.metaKeywords.map((kw, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 rounded-lg bg-[#012A22]/70 border border-[#3B796A]/40 text-[#ABCAC2] px-2.5 py-1 text-xs font-semibold shadow-xs"
                  >
                    <span>{kw}</span>
                    <button
                      type="button"
                      onClick={() => handleRemoveKeyword(idx)}
                      className="p-0.5 text-[#ABCAC2] hover:text-red-400 rounded transition-colors cursor-pointer"
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all cursor-pointer ${
                      isAlreadyAdded
                        ? "bg-slate-800/40 text-slate-600 border border-slate-800 cursor-default"
                        : "bg-slate-800/80 border border-slate-700/80 text-slate-300 hover:border-[#3B796A]/50 hover:text-white hover:bg-[#012A22]/40"
                    }`}
                  >
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#0e111a] p-5 sm:p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Social Sharing Cards (Open Graph &amp; Twitter)</h3>
          </div>
          <p className="text-xs text-slate-400">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-300">Open Graph Title (og:title)</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-bold text-slate-300">Twitter Card Title</label>
              <input
                type="text"
            ...
            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-300">Social Share Image URL (og:image)</label>
              <input
                type="text"
            ...
            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-300">Open Graph Description (og:description)</label>
              <textarea
                rows={2}
            ...
            <div>
              <label className="text-xs font-bold text-slate-300">Twitter Card Type</label>
              <select
                ...
                className="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white focus:border-[#012A22] focus:outline-none font-medium cursor-pointer"
              >
            ...
            <div>
              <label className="text-xs font-bold text-slate-300">Canonical Site Base URL</label>
              <input
                type="text"
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#0e111a] p-5 sm:p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Search Engine Crawlers &amp; Verification</h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-4 rounded-xl border border-slate-800 bg-slate-900/60">
              <div>
                <p className="text-xs font-bold text-white">Index Website (robots index)</p>
                <p className="text-[11px] text-slate-400">Allow search engines to index your pages</p>
              </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl border border-slate-800 bg-slate-900/60">
              <div>
                <p className="text-xs font-bold text-white">Follow Links (robots follow)</p>
                <p className="text-[11px] text-slate-400">Allow crawlers to follow internal page links</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#012A22] to-[#3B796A] hover:from-[#001F18] hover:to-[#012A22] text-white px-6 py-3 text-sm font-bold shadow-lg shadow-[#012A22]/30 transition-all cursor-pointer active:scale-98 disabled:opacity-60"
          >
            <Save className="h-4 w-4" />
